...
const UserService = require(`${__path_services}backend/user_service`);
...
module.exports = {
    getlist : async (req , res , next) => {
        ...
        let { categoryAccounts } = await UserService.getListCategory()
        ...
        let { data }  = await UserService.getAll({pagination, condition, sort})

        let choosedStatus = req.params.status;
        let statusFilter = await UserService.countAll({choosedStatus, arrIdCategory})
    ...
    getForm : async (req , res , next) => {
        ...
        let { categoryAccounts }   = await UserService.getListCategory()
        let { data, pageTitle } = await UserService.getForm({id})
    ...
    getStatus: async (req , res , next) => {
        ...
        let data = await UserService.changeStatus({id, status})
        data.status        = status
        data.id            = id
    ...
    getNumber: async (req, res, next) => {
        ...
        let {success} = await UserService.changeNumber({id, data})
    ...
    changeMultipleAction: async (req, res, next) => {
        ...
        if (action === 'delete') {
            let { deletedCount } = await UserService.ChangeDeleteMultiple({arrId})
            req.flash('success', util.format(notify.DELETE_MULTI_SUCCESS, deletedCount)) 
            res.redirect(`${linkPrefix}`)
        }else{
            let { modifiedCount } = await UserService.ChangeStatusMultiple({arrId, action})
    ...
    getCategory: async (req, res, next) => {
        ...
        let {success} = await UserService.changeCategory({id, data})
